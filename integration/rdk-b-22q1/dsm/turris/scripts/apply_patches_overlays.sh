...
#@description   Applies a patches/new files to add Dobby+DSM to an RDKB Turris Omnia build
#@param[in]     $1      target file directory path [NB: default is "."]
#@param[in]     $2      patch/new files base directory path containing folders "./patches" and "./new"  [NB: default is "./DSM/integration/rdk-b-22q1/dsm/turris"]
...
echo "====>>>"
echo "apply_patch: patch dir = $patch_dir"
echo "apply_patch: 1 = $1"
echo "apply_patch: 2 = $2"
echo "apply_patch: szukany plik = $patch_dir/$(basename $1).patch"
echo "patching : $2/$1"
...
#defaults
target=.
patch_new_files_dir=./DSM/integration/rdk-b-22q1/dsm/turris
...
echo "using newbies in $newbies "
   ...
   # dobby patches
apply_patch meta-turris/conf/distro/include/rdk-turris.inc $target $patches
apply_patch meta-turris/recipes-kernel/linux/linux-yocto_5.15.44.bb $target $patches
...
apply_patch meta-rdk-broadband/recipes-core/packagegroups/packagegroup-rdk-oss-broadband.bbappend $target $patches
    # dobby+dsm new files
apply_patch meta-turris/recipes-kernel/linux/linux-yocto-5.15/container.cfg $target $newbies
